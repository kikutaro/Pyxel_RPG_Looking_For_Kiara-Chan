    ...
    def draw(self):
        match self.モード:
            ...
            case self.モードメンバ:
                ...
                pyxel.text(15, 25,  self.選択したメンバー, 7, const.MESSAGE_FONT)
                ...
                pyxel.text(190, 25, "<<", const.色.RED.value if self.メニューIdx == 0 else const.色.WHITE.value, const.MESSAGE_FONT)
                pyxel.text(205, 25, "戻る", const.色.RED.value if self.メニューIdx == 1 else const.色.WHITE.value, const.MESSAGE_FONT)
                pyxel.text(230, 25, ">>", const.色.RED.value if self.メニューIdx == 2 else const.色.WHITE.value, const.MESSAGE_FONT)
                ...
                for i, (key, value) in enumerate(self.メンバープロフィール[self.選択したメンバー][1].items()):
                    if key == 'しゅみ':
                        for j, hobby in enumerate(value):
                            if j == 0:
                                pyxel.text(15, 65 + cnt * 15, f"{key}: {hobby}", const.色.WHITE.value, const.MESSAGE_FONT)
                            else:
                                pyxel.text(15, 65 + cnt * 15, f"     : {hobby}", const.色.WHITE.value, const.MESSAGE_FONT)
                            cnt += 1
                    elif key == 'とくぎ':
                        for j, skill in enumerate(value):
                            if j == 0:
                                pyxel.text(15, 65 + cnt * 15, f"{key}: {skill}", const.色.WHITE.value, const.MESSAGE_FONT)
                            else:
                                pyxel.text(15, 65 + cnt * 15, f"     : {skill}", const.色.WHITE.value, const.MESSAGE_FONT)
                            cnt += 1
                    else:
                        pyxel.text(15, 65 + cnt * 15, f"{key}: {value}", const.色.WHITE.value, const.MESSAGE_FONT)
                        cnt += 1
